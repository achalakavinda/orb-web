	...
	$(function() {
		...
		// Main Sections: Two.
			...
			function Update() {
                ...
                result = calculateCost(OrbChargeGBSecond, OrbRequestCharge, OrbFreeTier, OrbFreeRequests, OrbHTTPCharge);
                
                if (result.executionCost && result.requestCost && result.totalCost) {
					$('#orb-execution-cost').text(parseFloat(result.executionCost).toFixed(2));
					$('#orb-request-cost').text(parseFloat(result.requestCost).toFixed(2));
					$('#orb-total-cost').text(parseFloat(result.totalCost).toFixed(2));
                    updatePie();
				}
